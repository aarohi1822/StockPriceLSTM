...
y_test_real = scaler.inverse_transform(np.hstack([y_test.reshape(-1,1), np.zeros((y_test.shape[0], len(features)-1))]))[:,0]
y_pred_real = scaler.inverse_transform(np.hstack([y_pred, np.zeros((y_pred.shape[0], len(features)-1))]))[:,0]
...
# Plot prediction vs actual with signals
...
buy_points = np.where(np.array(signals)==1)[0]
sell_points = np.where(np.array(signals)==-1)[0]
plt.scatter(buy_points, y_test_real[buy_points], marker="^", color="green", label="BUY")
plt.scatter(sell_points, y_test_real[sell_points], marker="v", color="red", label="SELL")
plt.legend()
plt.xlabel("Days")
plt.ylabel("Price")
st.pyplot(plt.gcf())
...
plt.figure(figsize=(12,5))
plt.plot(future_30, marker='o', color='blue', label="Future Prices")
plt.xlabel("Days")
